...
const MIGRATION_QUERIES = [
  ...
  `CREATE POLICY tenant_isolation_workspaces ON workspaces FOR ALL USING (org_id = CURRENT_SETTING('app.current_tenant_id', true));`,
  `CREATE POLICY tenant_isolation_links ON links FOR ALL USING (org_id = CURRENT_SETTING('app.current_tenant_id', true));`,
  `CREATE POLICY tenant_isolation_smart_routes ON smart_routes FOR ALL USING (org_id = CURRENT_SETTING('app.current_tenant_id', true));`,
  `CREATE POLICY tenant_isolation_retargeting_pixels ON retargeting_pixels FOR ALL USING (org_id = CURRENT_SETTING('app.current_tenant_id', true));`,
  `CREATE POLICY tenant_isolation_click_events ON click_events FOR ALL USING (org_id = CURRENT_SETTING('app.current_tenant_id', true));`
]
...
export async function runDatabaseMigration(databaseUrl: string) {
  ...
  for (const query of MIGRATION_QUERIES) {
    try {
      ...
      // Continue if policy already dropped or table exists
    }
  }
}
